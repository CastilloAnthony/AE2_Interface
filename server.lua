            ...
            if message['verify']['id'] == i['id'] then
              if message['verify']['label'] == i['label'] then
                ...
                elseif packet['type'] == 'allData' then
                  server.modem.transmit(28, 0, {['message'] = 'Enjoy!', ['verify'] = server.getComputerInfo(), ['target'] = message['verify'], ['encryptionTest'] = crypt.encrypt(i['sharedKey'], 'Valid'), ['packet'] = textutils.serialize({['type'] = 'allData', ['data'] = server.snapshotItems}, {allow_repetitions = true})})
